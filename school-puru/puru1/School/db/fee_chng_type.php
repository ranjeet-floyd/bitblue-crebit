<?php
include('../../db.php');
$result=mysqli_fetch_row(mysqli_query($con,"Select fee,lfee,one_time from sch_cls_fee where Medium='".trim($_POST['fee_chng_mdm'])."' AND Std='".trim($_POST['fee_chng_std'])."' And fee_type='".trim($_POST['fee_chng_type'])."'"));
if($result){
	//print_r($result[2]);
	if($result[2]=='Per Year'){
		...
		$mon=array('Term 1','Jun','Jul','Aug','Sep','Oct','Term 2','Nov','Dec','Jan','Feb','Mar','Apr','May');
